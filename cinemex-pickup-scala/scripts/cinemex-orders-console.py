...
def flatten_orders(response, limit_per_page):
    ...
    for order in response['data']:
        array_aplanado.extend([
            order.get('customerName', u''),  # No codifiques aquí
            order.get('id', u''),
        ])
...
response_ordenes_pendientes = getOrders(pickup_api_url, player_ip_A1, player_port_A1, player_user_A1, player_pass_A1, hostname, page_pending, limit=PENDING_ORDERS_LIMIT, token=access_token)
...
response_ordenes_completadas = getOrders(pickup_api_url, player_ip_A1, player_port_A1, player_user_A1, player_pass_A1, hostname, page_completed, limit=COMPLETED_ORDERS_LIMIT, token=access_token, preparationStatus=5)  # 5 es el estatus de orden completada
...
# print("Numero de ordenes pendientes: " + str(response_ordenes_pendientes["count"]))
print("Respuesta de la API de ordenes pendientes:\n" + json.dumps(response_ordenes_pendientes, indent=2, ensure_ascii=False))

# print("Numero de ordenes completadas: " + str(response_ordenes_completadas["count"]))
